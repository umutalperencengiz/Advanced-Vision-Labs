...
class Network(torch.nn.Module):
	def __init__(self):
		...
		self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.hub.load_state_dict_from_url(url='http://content.sniklaus.com/github/pytorch-liteflownet/network-' + 'default' + '.pytorch').items() })
	# end
...
tenOne = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open('pedestrian/input/in%06d.jpg' % 1))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
iStep = 1

if __name__ == '__main__':
	for i in range(2,1100,iStep):
		tenTwo = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open("pedestrian/input/in%06d.jpg" % i))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
		...
		flow = tenOutput.numpy().transpose(1, 2, 0)
		mag, ang = cv2.cartToPolar(flow[:,:,0],flow[:,:,1])
		mag[0][0] = 10
		hsv = numpy.zeros((tenOutput.shape[1], tenOutput.shape[2],3),numpy.float32)
		hsv[..., 2] = 255
