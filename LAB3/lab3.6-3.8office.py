...
if __name__ == "__main__":
    ...
    bg_subtractorSpecial = cv2.createBackgroundSubtractorMOG2(history=2000,varThreshold=10,detectShadows=False)
    bg_subtractor_KNN = cv2.createBackgroundSubtractorKNN(history = 2000,dist2Threshold=50)
    kernel = np.ones((5, 5), np.uint8)
    with open("office/temporalROI.txt", "r") as f:
        line = f.readline()
        roi_start, roi_end = map(int, line.split())
    for i in range(roi_start,roi_end):
        FI = cv2.imread("office/input/in%06d.jpg" %i,cv2.IMREAD_GRAYSCALE)
        GI = cv2.imread("office/groundtruth/gt%06d.png" %i,cv2.IMREAD_GRAYSCALE)
