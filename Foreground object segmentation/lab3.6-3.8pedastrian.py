...
if __name__ == "__main__":
    ...
    bg_subtractorSpecial = cv2.createBackgroundSubtractorMOG2(detectShadows=False)
    ...
    F1I = cv2.imread("pedestrian/input/in%06d.jpg" % roi_start ,cv2.IMREAD_GRAYSCALE)
    for i in range(roi_start,roi_end):
        ...
        _, thresh_MOG= cv2.threshold(fg, 30, 255, cv2.THRESH_BINARY)
        thresh_subtractor = cv2.morphologyEx(thresh_MOG, cv2.MORPH_OPEN, kernel)
        #Special Values
        fgSpecial= bg_subtractorSpecial.apply(FI)
        _, thresh_MOG_special= cv2.threshold(fgSpecial, 30, 255, cv2.THRESH_BINARY)
